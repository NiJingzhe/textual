...
class TailwindTCSS:
    ...
    def _compile_token(self, token: str) -> tuple[CompiledUtility, bool]:
        ...
        utility_name, raw_value = self._parse_arbitrary_utility(token, base_utility)
        ...
        normalized_lines = self._normalize_declarations(
            token, declaration_name, raw_value
        )
    ...
    @staticmethod
    def _parse_arbitrary_utility(token: str, base_utility: str) -> tuple[str, str]:
        match = _ARBITRARY_UTILITY_RE.fullmatch(base_utility)
        if match is None:
            raise ValueError(
                f"Unsupported utility token {token!r}; use arbitrary-value syntax like 'w-[42]'"
            )
