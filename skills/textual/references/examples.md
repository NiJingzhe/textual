...
## Table of Contents
- [Tailwind Utility App](#tailwind-utility-app)
...
class TailwindApp(App):
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        with Vertical(classes="gap-2 p-4 bg-slate-900 border-round"):
            yield Label("Tailwind-Styled App", classes="text-center font-bold text-slate-50 text-[#f8fafc]")
            with Horizontal(classes="gap-2 align-center"):
                yield Button("Primary", classes="bg-blue-500 text-white hover:bg-blue-700 transition-colors cursor-pointer")
                yield Button("Success", classes="bg-emerald-500 text-white hover:bg-emerald-700 transition-colors cursor-pointer")
                yield Button("Danger", classes="bg-red-500 text-white cursor-pointer transition-colors hover:opacity-[80%]")
            with Container(classes="border p-2 bg-slate-700 text-slate-50"):
                yield Label("Uses bundled tailwind.tcss utility sheet")
                yield Label("Arbitrary: w-[42] bg-[#334155] opacity-[55%]")
        yield Footer()
...
No external CSS file needed - all styling is via utility classes.
